...
class BaseGridSearch(_DelegatedForecaster):
    ...
    def _fit(self, y, X, fh):
        ...
        def evaluate_candidates(candidate_params):
            ...
            # Handle dask_lazy backend: conditionally materialize or keep lazy
            if backend == "dask_lazy":
                if self.materialize_dask_lazy:
                    # Materialize immediately (default behavior)
                    from dask import compute

                    out = list(compute(*out, **backend_params))
                else:
                    # Store lazy objects for external orchestration
                    # but still materialize for internal processing
